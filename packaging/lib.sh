...
# manifest, the spec, the AppStream release list and `videonsole --version`
...
PACKAGE_WORK_ROOT="${VIDEONSOLE_WORK_DIR:-$PACKAGING_DIR/out/build}"
...
# A relative VIDEONSOLE_WORK_DIR has to become absolute here rather than at
...
if [[ -n "${VIDEONSOLE_WORK_DIR:-}" ]]; then
    package_set_work_root "$VIDEONSOLE_WORK_DIR"
fi
...
Point the build somewhere with room using --work-dir DIR (or VIDEONSOLE_WORK_DIR),
