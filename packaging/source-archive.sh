...
output="${1:-$PACKAGING_DIR/out/sources/videonsole-$PACKAGE_VERSION.tar.gz}"
if [[ "$output" != /* ]]; then
    output="$PWD/$output"
fi

work="$(package_work_dir videonsole-source)"
cleanup() {
    if [[ -n "${work:-}" && "$work" == */videonsole-source.* && -d "$work" ]]; then
        rm -rf -- "$work"
    fi
}
trap cleanup EXIT
